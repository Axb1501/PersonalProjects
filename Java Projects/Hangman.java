...
public class Hangman {
	...
	int correct = target.indexOf(letter);
	if (correct > - 1) {
		return true;}
	else
		{return false;}
	...
	public String getCurrentGuessForDisplay(String currentGuess) {
		...
		String show ="";
		String display = currentGuess.replace('*','_');
		for (int i = 0; i < currentGuess.length() + 1; i++) {
			if (i > 0 && i < currentGuess.length() + 1) {
				show += display.substring(i - 1,i);
				show += " ";}
			else if (i == currentGuess.length()) {
				show += display.charAt(i);}
			else {
				;}
	...
	if (target.indexOf(letter) > -1){
		for (int i = 0; i < guess.length(); i++) {
			...
			{
			    open += target.charAt(i);
			    getCurrentGuessForDisplay(open);}
